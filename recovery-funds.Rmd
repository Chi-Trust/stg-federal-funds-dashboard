...
## Percent of obligated funds spent
...
plot_expended_slfrf <- function(.data, geography) {
  .data |>
    ...
    summarise(
      pct_expended = mean(pct_expended, na.rm = TRUE),
      .by = c("geography", "topic")
    ) |>
    filter(geography == {{ geography }}) |>
    ggplot() +
    geom_col(mapping = aes(x = topic, y = pct_expended), fill = "#47c3d3") +
    coord_flip() +
    scale_y_continuous(labels = scales::label_percent()) +
    labs(
      title = "Percent of State and Local Fiscal Recovery Funds Expended by Topic",
      x = NULL,
      y = NULL
    )
}
