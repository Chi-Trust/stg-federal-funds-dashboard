...
plot_funds_by_geo_topic <- function(data, fill_var){
 ...
 if (fill_var == "legislation"){
   ...
   fill_values <- c("#47c3d3", "#C1D82F", "#6C5893", "#000000", 
                    "#387ECF", "#FDBB30", "grey")[1:length(fill_breaks)]
...
## Compare spending between the original allocations and spending to date by policy topic {.tabset .tabset-pills}
...
plot_difference_in_allocation_vs_spending <- function(data){
    
  original_allocations <-
    data %>% 
    summarise(allocation = sum(allocation, na.rm = TRUE), .by = c(topic)) %>%
    mutate(type = "Original Allocations",
           pct_spend = allocation/sum(allocation)) %>%
    select(type, topic, pct_spend)
  
  spending_to_date <- 
    data %>% 
    summarise(total_cumulative_expenditures = sum(total_cumulative_expenditures,
                                                  na.rm = TRUE), 
              .by = c(topic)) %>%
    mutate(type = "Cumulative Spending",
           pct_spend = total_cumulative_expenditures/sum(total_cumulative_expenditures)) %>%
    select(type, topic, pct_spend)
  ...
  original_allocation_vs_spending <- rbind(original_allocations, spending_to_date)
  
 p <- ggplot(original_allocation_vs_spending) +
    geom_col(aes(x = reorder(topic, pct_spend), y = pct_spend, fill = factor(type)), 
             position = "dodge") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.01)), 
    ...
    #                      breaks = fill_breaks)+
    labs(x = "Topic", 
         y = "Expenditures to date relative to original allocation (percentage)",
         caption = "This graph compares the percent of ARPA SLFRF dollars by policy topic between the original allocations and spending to date") +
    coord_flip() +
    theme(text = element_text(family = "Merriweather",
                              face = "plain",
                              colour = "#000000",
...
```{r}
#map(c("Chicago", "Cook County", "Illinois"), ~obligated_funds_by_geo(.x))

```

### Chicago
```{r}
obligated_funds_by_geo("Chicago")
```

### Cook County

```{r}
obligated_funds_by_geo("Cook County")
```

### Illinois
```{r}
obligated_funds_by_geo("Illinois")
```
...
format_write_data_download <- function(data_filter, geo_format, policy_name){
  ...
  put_object(file = here("data/dashboard_downloads", str_glue("{policy_name}_{geo_format}.zip")),
             bucket = bucket_name,
             object = str_glue("{policy_name}_{geo_format}_stg.zip"))
...
chic_rec_alt <- map(c("Chicago General Obligation Bond", 
                                                      "ARPA State and Local Fiscal Recovery Funds"), ~make_treemap_alt_text(data_all_filter, 
                                      geo = "Chicago", 
                                      leg_name =  .x))
