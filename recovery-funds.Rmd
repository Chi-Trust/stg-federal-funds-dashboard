...
#                   sheet = "Programs") %>%
data_all <- read_csv(here("data", "cct_dashboard_data.csv"),locale = locale(encoding = 'UTF-8')) %>%
  janitor::clean_names() %>%
  ...
  mutate(
    ...
    across(c(allocation, adopted_budget_per_treasury_report, total_cumulative_obligation, total_cumulative_expenditures, amount_exact),
           ~ parse_number(.x)),
...
data_filter <- data %>%
  ...
  select(legislation, program, topic = new_topic, subtopic = new_subtopic, allocation)
...
## Compare spending between the original recovery plan and spending to date by policy topic {.tabset .tabset-pills}
...
plot_difference_in_allocation_vs_spending <- function(data){
  ...
  original_allocations <-
    ...
    mutate(type = "Chicago Recovery Plan",
 ...
 p <- ggplot(original_allocation_vs_spending) +
    ...
    labs(x = "Topic", 
         ...
         caption = "This graph compares the percent of ARPA SLFRF dollars by policy topic between the original recovery plan and spending to date") +
    coord_flip() +
    theme(text = element_text(family = "Merriweather",
                              face = "plain",
                              colour = "#000000",
...
data_all %>%
  ...
  select(geography, legislation, topic = new_topic, subtopic = new_subtopic, pct_obligated_funds_spent) %>% 
