 ...
 data <- data %>%
  ...
  fill_breaks <- data %>%
   ...
   if (length(unique(data$ffy)) > 1) {
     funding_sentence = "This chart shows a combination of actual federal funding\nwhere the specific recipient and use are determined (awards) for previous fiscal years\nand expected federal funding where the specific recipient and use may not be determined (allocations)\nfor the current fiscal year."
     
     ylab = "Total Actual and Expected Funding (millions)"
     
   } else if(max(data$ffy) == cur_fiscal_year) {
     funding_sentence = "This chart shows expected federal funding\nwhere the specific recipient and use may not be determined (allocations) for the current fiscal year."
     
     ylab = "Total Expected Funding (millions)"
     
   } else {
     funding_sentence = "This chart shows actual federal funding\nwhere the specific recipient and use are determined (awards) for previous fiscal years."
   ...
   plt <- ggplotly(allocations_by_recipient_var, tooltip = "text") %>%
     layout(autosize = F, 
            ...
            annotations = list(
              x = 0,
              y = -0.6,
              text = str_glue("This graph shows selected projects funded through the Infrastructure Investment and Jobs Act.\nFor more detail, see the FAQ.{funding_sentence}"),
